...
for c in Ωc:
    for s in Ωs:
        contingency_results[(c,s)] = {
            'PS': [model.PEDS[t, c, s].value for t in Ωt],
            ...
            'Demand': [data["EDS"]["LOAD"] * fs[s]['load'][t] for t in Ωt],
...
for c in Ωc:
    for s in Ωs:
        ...
        PS_values = contingency_results[(c,s)]['PS']
        ...
        PV_generated = contingency_results[(c,s)]['PV']
        demand_values = contingency_results[(c,s)]['Demand']
        ...
        ax.plot(Ωt, PS_values, label='EDS Power', color=colors['Substation'], linewidth=2)
        ax.plot(Ωt, demand_values, label='Demand', color=colors['Demand'], linewidth=2, linestyle='--', marker='o', markersize=1) 
        ax.plot(Ωt, PV_generated, label='Solar Generation', color=colors['Solar'], linewidth=2)
        ax.plot(Ωt, PEV_c_values, label='Total EV charging Power', color=colors['Total EV charging Power'], linewidth=2)
        ax.plot(Ωt, PEV_d_values, label='Total EV discharging Power', color=colors['Total EV discharging Power'], linewidth=2)
...
for c in Ωc:
    for s in Ωs:
        ...
        ax.set_title(f"EV & BESS SoC c/ αEV - Contingency {c} Scenario {s}", fontsize=14, fontweight='bold')

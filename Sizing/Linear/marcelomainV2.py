...
model.eds_power_constraint = Constraint(Ωt, rule=eds_power_rule)

# Define the rule for the PSp (charging power) constraint
def psp_constraint(model, t):
    return model.PSp[t] <= data["EDS"]['Pmax']
psp_constraint = Constraint(Ωt, rule=psp_constraint)
...
def ev_power_rule_c(model, ev, t):
    return model.PEV_c[ev, t] <= model.αEV[ev, t] * EV[ev]['Emax']
model.ev_power_c = Constraint(Ωev, Ωt, rule=ev_power_rule_c)

def ev_power_rule_d(model, ev, t):
    return model.PEV_d[ev, t] <= model.αEV[ev, t] * EV[ev]['Emax']
model.ev_power_d = Constraint(Ωev, Ωt, rule=ev_power_rule_d)
...
sum_PEV_values_d = [
    sum(- model.PEV_d[ev, t].value for ev in Ωev) for t in Ωt
] # Sum of PEV values for each time step

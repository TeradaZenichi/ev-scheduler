...
# with open(f'{folder}/EVs_clean.json') as file:
EV = {}
...
def generate_random_profile(Δt=15):
    """
    Gera um perfil de carregamento de veículo elétrico com chegada e saída dentro de um período aleatório
    (madrugada, dia ou noite), garantindo que os horários sejam múltiplos de Δt.
    
    Δt: Intervalo de tempo em minutos (default = 15 minutos)
    """
    # Escolher aleatoriamente um período com distribuição equilibrada
    period = random.choice(["midnight", "day", "night"])

    if period == "night":  # Noite (18:00 - 00:00)
        arrival_hour = random.randint(18, 23)
    elif period == "midnight":  # Madrugada (00:00 - 06:00)
        arrival_hour = random.randint(0, 5)
    else:  # Dia (06:00 - 18:00)
        arrival_hour = random.randint(6, 17)

    # Converter hora para minutos e ajustar para múltiplos de Δt
    arrival_minutes = (arrival_hour * 60) // Δt * Δt

    min_duration = (2 * 60) // Δt * Δt  # 2 horas em minutos, ajustado para Δt
    max_duration = (6 * 60) // Δt * Δt  # 6 horas em minutos, ajustado para Δt
    duration = random.randint(min_duration // Δt, max_duration // Δt) * Δt

    departure_minutes = arrival_minutes + duration

    # Ajuste para quando ultrapassa meia-noite
    if departure_minutes >= 24 * 60:
        departure_minutes -= 24 * 60

    # Converter minutos de volta para formato HH:MM
    arrival_time = (datetime.strptime('00:00', "%H:%M") + timedelta(minutes=arrival_minutes)).strftime("%H:%M")
    departure_time = (datetime.strptime('00:00', "%H:%M") + timedelta(minutes=departure_minutes)).strftime("%H:%M")

    return {
        "arrival": arrival_time,
        "departure": departure_time,
        "SoCini": round(random.uniform(0.1, 0.5), 2),
        "Emax": random.choice([35, 40, 45, 50]),
        "eff": 0.95,
        "Pmax_c": random.choice([20, 25, 30, 35]),
        "Pmax_d": random.choice([10, 15])
    }
